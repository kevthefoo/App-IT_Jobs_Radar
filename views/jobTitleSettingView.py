...
class jobTitleSettingView(View):
    def __init__(self, user, main_guild):
        self.user = user
        self.main_guild = main_guild
        super().__init__(timeout=None)
        with open('./data/views/jobTitleSettingData.json', 'r') as f:
            data = json.load(f)
    ...
    async def buttonCallback(self, interaction: discord.Interaction):
        ...
        if role not in self.user.roles:
            ...
            await interaction.response.edit_message(content=f"**Select job titles you want to apply**", view=self)
        else:
            for button in all_buttons:
                if button.label == custom_id:
